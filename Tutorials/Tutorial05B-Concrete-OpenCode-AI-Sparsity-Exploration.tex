...
\begin{enumerate}[nosep]
  ...
  \item Type the provided OpenCode prompt in the message box 
  \begin{itemize}
        \item Press \key{Enter} to send the prompt
        \item Press \key{Shift+Enter} to change line
        \item Use basic Markdown formats (e.g., **text** for bold fonts; - for unordered list, \texttt{`code`} for codes, etc.)
  \end{itemize}
...
\textbf{Objective:} Identify outliers in strength using the \href{https://en.wikipedia.org/wiki/Interquartile_range}{IQR method}.
