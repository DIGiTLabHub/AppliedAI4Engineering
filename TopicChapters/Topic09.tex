...
\title{CE 488/5588: Applied Machine Learning\\ \large Topic 09 (optional): Gaussian Classifier and Naive Bayes Classifier}
\author{}
\date{}
\maketitle


This optional topic introduces two generative classification frameworks: Gaussian classifiers and Naive Bayes classifiers. We first derive classification rules from class-conditional densities and class priors, then discuss parameter estimation through Maximum Likelihood Estimation (MLE). We also summarize key Naive Bayes variants (Gaussian, Multinomial, and Bernoulli) and close with practical considerations such as smoothing and model assumptions.
...
In this topic, we review the fundamentals of modeling \(p(\bm{x}|c_i)\) using multivariate Gaussian and Naive Bayes models. We do not cover Bayesian networks in detail; however, Naive Bayes can be viewed as one of the simplest Bayesian network models.
...
In Gaussian-distribution-based binary classification, we consider two classes, \(c_1\) and \(c_2\). The class-conditional densities \(p(\bm{x} | c_1)\) and \(p(\bm{x} | c_2)\) follow Gaussian distributions. We now discuss the role of covariance matrices and how to evaluate posterior probabilities for binary classification.
...
\begin{equation}
g(\bm{x}) = -\frac{1}{2}(\bm{x} - \bm{\mu}_1)^T \bm{\Sigma}_1^{-1} (\bm{x} - \bm{\mu}_1) + \frac{1}{2}(\bm{x} - \bm{\mu}_2)^T \bm{\Sigma}_2^{-1} (\bm{x} - \bm{\mu}_2) + \frac{1}{2}\log \frac{|\bm{\Sigma}_2|}{|\bm{\Sigma}_1|} + \log \frac{p(c_1)}{p(c_2)}
\end{equation}
...
In summary, for Gaussian-based binary classification, we use the covariance matrices \(\bm{\Sigma}_1\) and \(\bm{\Sigma}_2\) together with Gaussian densities to compute (log-)posterior probabilities and classify new inputs.
...
If \(p(\bm{x}|c_1)\) and \(p(\bm{x}|c_2)\) are Gaussian with known means and covariances, and the class priors \(P(c_1)\) and \(P(c_2)\) are known, we can define the theoretical Bayes error. This error represents the minimum possible misclassification rate determined by the true underlying distributions.
...
Naive Bayes is a popular probabilistic classifier based on Bayes' theorem. The term "naive" refers to the assumption that features are conditionally independent given the class label---a simplification that rarely holds perfectly in practice but often yields competitive performance.
...
\begin{enumerate}
    \item \textbf{Smoothing (Handling Unseen Features):} \\
    ...
    For continuous features, normalization or scaling can improve numerical stability and make feature magnitudes easier to interpret.
