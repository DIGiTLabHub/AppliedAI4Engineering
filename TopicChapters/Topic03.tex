...
\noindent
This topic provides a high-level map of machine learning within modern AI. We introduce the three core learning paradigms (supervised, unsupervised, and reinforcement learning) and two complementary mathematical viewpoints for building learning systems: discriminative and generative learning. The goal is to clarify what is being learned in each setting, what assumptions are made about data and uncertainty, and how these choices shape modeling and inference in later topics.

\paragraph{Learning Objectives} After completing this chapter, you should be able to:
\begin{itemize}
    \item Describe machine learning as data-enabled AI and distinguish training (induction) from inference (deduction).
    \item Identify and compare the three fundamental ML paradigms: supervised learning, unsupervised learning, and reinforcement learning.
    \item Explain the roles of datasets, labels, and interaction-generated data in each paradigm, and recognize common hybrid variants (e.g., semi-supervised and self-supervised learning).
    \item Define discriminative learning and generative learning, and state what each approach models (e.g., $P(c\mid x)$ vs.\ $p(x\mid c)$ and $P(c)$).
    \item Use Bayes' rule to connect generative models to classification and interpret posterior probabilities for decision making.
    \item Apply the paradigms and formalisms to simple engineering examples (e.g., water-quality compliance) to articulate modeling assumptions and limitations.
\end{itemize}
