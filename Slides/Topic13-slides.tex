...
\begin{warningbox}{What is a Token?}

\textbf{Plain language:}  
A token is a small unit of data that the model processes.

\textbf{Examples:}  
\begin{itemize}

    \item Text: \texttt{"Hello world"} $\rightarrow$ ["Hello", "world"]  
    \item Image: an image $\rightarrow$ small patches (tokens)
...
\begin{frame}{What Makes These Architectures Generative?}
  \begin{conceptbox}{Architecture versus Modeling Goal}
...
\begin{frame}{What Makes These Architectures Generative?}
  ...
  \begin{itemize}
    \item CNNs become generative inside when integrating decoders; if just encoders or encoders-like, it is for classification or feature extraction, as in classic ML classification or clustering. 
    
  \begin{itemize}
        \item U-Nets: Encoder–decoder CNNs that generate structured outputs conditioned on input.
        \item GAN: Uses a generator (decoder) – discriminator (encoder-like) setup to produce realistic new data from noise
    \end{itemize}
    
    \item Transformers have built-in encoders or/and decoders, becoming generative when they generate sequences token by token.
    \item Diffusion models are generative because they learn a sampling process from noise to data.
  \end{itemize}
  
\end{frame}

\begin{frame}{Old Generative Models versus New Generative AI}
  In classical machine learning, a model was called \textbf{generative} if it explicitly modeled how data were produced.

  \begin{itemize}
    \item Examples: Gaussian mixture models, hidden Markov models, Bayesian latent-variable models.
    \item Modern deep generative AI keeps that goal, but uses neural networks to synthesize complex high-dimensional outputs directly.
  \end{itemize}

  \begin{conceptbox}{The New Emphasis}
...
\begin{frame}{Regardless,}
  \begin{itemize}
    \item A \textbf{discriminative} model asks: given this input, what label, class, or value should I predict?
    \item A \textbf{generative} model asks: what plausible sample could have come from this data-generating process?
    \item For engineers, this means AI can support not only classification and forecasting, but also synthesis, reconstruction, and design exploration.
  \end{itemize}
